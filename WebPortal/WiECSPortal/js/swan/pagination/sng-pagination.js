  		...
  		StandardSetup.prototype.link = function($scope, $element, $attributes, $filter, $http, table) {    	
  			...
  			var update = function() {
  				...
  				if ( $scope[config].pageFront && totalList.length <= 0 
  					...
  					if($scope[config].method == 'GET'){
  						httpConfig.params = $scope[config].formData;
  					}else if($scope[config].method == 'POST'){
  						httpConfig.data = $scope[config].formData;
  					}
  					...
  					/*$http({
  	  					...
  	  					if( (data.status == $scope[config].successStatus) && (data[list].length > 0)) {  	  				
  	  						$scope[config].tableError = false;
							if(typeof $scope[config].changeDataFn === 'function'){
		  							tableData = $scope[config].changeDataFn(data[list]);
		  						}else tableData = data[list];
