...
function getLinkFunction($http, $timeout, theme, util, type) {
    return function (scope, element, attrs) {
        ...
        function getSizes(config) {
            width = config.width || parseInt(attrs.width) || ndParent.clientWidth || 320;
            height = config.height || parseInt(attrs.height) || ndParent.clientHeight || 240;
        ...
        function getOptions(data, config, type) {
            ...
            if(config.xAxis instanceof Array) {
            	...
            	options.xAxis = [ angular.extend(xAxis, util.getAxisTicks(data, config, type)) ];
            }
            if(config.yAxis instanceof Array) {
            ...
            if(config.series instanceof Array) {
            	options.series = config.series;
            }else{
            	options.series = util.getSeries(data, config, type);
            }
            if (!config.showXAxis) {
            ...
            if(config.dataZoom instanceof Array) {
        ...
        scope.$on('reloadEChartForConfig', function(event,data) {
        	setOptions();
        });
    };
}
